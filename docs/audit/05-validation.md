...
- Après `07ccfd2`, GitHub a signalé l'exécution forcée sous Node 24 d'actions encore fondées sur Node 20. Les références concernées passent aux versions officielles Node 24 (`checkout` v7, `setup-node` v7, `upload-pages-artifact` v5, `deploy-pages` v5 et `upload-artifact` v7) ; les workflows doivent être rejoués avant de clore ce point.
