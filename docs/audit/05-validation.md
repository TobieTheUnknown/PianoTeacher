...
- Contrôle visuel portrait sur le Pixel : dock d'onglets visible dans Live, absence de bande vide entre transport et onglets du Coach, cartes de chaque rangée alignées, et motifs répétés des cartes encadrés avec le même composant pour les deux mains.
...
2. Reconnecter le Pixel, installer l'APK avec `adb install -r`, puis écouter transitions de mesures, accords simultanés, notes répétées, boucle, scrub Live et reprise après une autre application audio.
